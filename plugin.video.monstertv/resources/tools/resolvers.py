# -*- coding: utf-8 -*-
#------------------------------------------------------------
# MonsterTV (juarrox@gmail.com)
# Conectores multimedia para MonsterTV
#------------------------------------------------------------
...
def allmyvideos(params):
    ...
    if "<b>File Not Found</b>" in data or "<b>Archivo no encontrado</b>" in data or '<b class="err">Deleted' in data or '<b class="err">Removed' in data or '<font class="err">No such' in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Archivo borrado!", 3 , art+'icon.png'))
...
def streamcloud(params):
    ...
    progreso.create("MonsterTV", "Abriendo Streamcloud..." , url )
...
def playedto(params):
    ...
    if body == "<center>This video has been deleted. We apologize for the inconvenience.</center>":
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Enlace borrado...", 3 , art+'icon.png'))
    elif body.find("Removed for copyright infringement") >= 0:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Removed for copyright infringement", 3 , art+'icon.png'))
    else:
        r = re.findall('file(.+?)\n', body)
        ...
        for entry in r:
            ...
            if entry.endswith("flv"):
                ...
                xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Resolviendo enlace...", 3 , art+'icon.png'))
                params["url"]=entry
...
def nowvideo(params):
    ...
    if "The file is being converted" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "El archivo está en proceso", 3 , art+'icon.png'))
    elif "no longer exists" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "El archivo ha sido borrado", 3 , art+'icon.png'))        
    else:
        #plugintools.log("data= "+data)
        ...
        if video_id == "":
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Error!", 3 , art+'icon.png'))
        else:
            ...
            if len(body) >= 0:
                ...
                url = body[0]
                plugintools.play_resolved_url(url)
                xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Cargando vídeo...", 1 , art+'icon.png'))
            else:
                xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Error!", 3 , art+'icon.png')) 
...
def veehd(params):
    ...
    if uname == '' or pword == '':
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Debes configurar el identificador para Veehd.com", 3 , art+'icon.png'))
        return
    ...
    if not vpi:
        if 'type="submit" value="Login" name="submit"' in body:
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Error al identificarse en Veehd.com", 3 , art+'icon.png'))
        else:
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Error buscando el video en Veehd.com", 3 , art+'icon.png'))            
        return
    ...
    if not video_url:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Error abriendo el video en Veehd.com", 3 , art+'icon.png'))
        return
...
def mediafire(params):
    ...
    plugintools.log("[MonsterTV] Espere un segundo...")
    import time
    time.sleep(1)
    data = plugintools.read(url)
    plugintools.log("data= "+data)
...
def novamov(params):
    ...
    if "This file no longer exists on our servers" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "No existe vídeo en Novamov", 3 , art+'icon.png'))
    elif "is being converted" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Vídeo no disponible", 3 , art+'icon.png'))
...
def videobam(params):
    ...
    if "Video is processing" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('MonsterTV', "Archivo no disponible temporalmente!", 3 , art+'icon.png'))
    else:
        patronHD = " high: '([^']+)'"
...
def flashx(params):
    ...
    #http://flashx.tv/embed-hhbhukuonni8-640x360.html
    #http://www.flashx.tv/rsrzvhrr38qn.html
